...
import { ImportAction } from '@chris.araneo/yet-another-todo-app-models';
import {
  createTask,
  hideTask,
...
export class TasksService implements OnDestroy {
  ...
  getHiddenTasks(): Observable<Task[]> {
    const hiddenTasksFilter = map((tasks: Task[]) =>
      tasks.filter((task) => task.getIsHidden()),
    );
  ...
  addTask(task: Task): Observable<HttpLogItem | undefined> {
    ...
    const responseObservable = this.getResponseObservable(
      operationId,
      'post',
      'task',
    );
  ...
  private subscribeToUserLoggedIn(): void {
    const subscription = this.userService
      ...
      .subscribe((isLogged) => {
        if (isLogged) {
          from(
            this.apiClientService.fetchTasksFromApi(
              this.operationIdGeneratorService.generate(),
            ),
          )
            .pipe(
              first(),
