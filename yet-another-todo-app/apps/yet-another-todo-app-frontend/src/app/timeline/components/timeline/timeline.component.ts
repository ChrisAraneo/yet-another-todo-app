...
export class TimelineComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  startDate: BehaviorSubject<Date | null> = new BehaviorSubject<Date | null>(
    null,
  );
  endDate: BehaviorSubject<Date | null> = new BehaviorSubject<Date | null>(
    null,
  );
  ...
  update(startDate?: Date, endDate?: Date): void {
    ...
    if (!isNaN(daysInPeriodOfTime) && daysInPeriodOfTime > 10 * 365) {
      throw new Error('Selected too many days to display on the timeline');
    }
  ...
  changeStartDateToPreviousMonth(): void {
    ...
    if (startDate instanceof Date) {
      const firstDayOfPreviousMonth =
        this.dateUtils.getFirstDayOfThePreviousMonth(startDate);

      this.changeStartDate(firstDayOfPreviousMonth);
      this.changeEndDate(
        this.dateUtils.getLastDayOfTheMonth(firstDayOfPreviousMonth),
      );
    }
  }
  ...
  changeStartDateToNextMonth(): void {
    ...
    if (startDate instanceof Date) {
      const firstDayOfNextMonth =
        this.dateUtils.getFirstDayOfTheNextMonth(startDate);

      this.changeStartDate(firstDayOfNextMonth);
      this.changeEndDate(
        this.dateUtils.getLastDayOfTheMonth(firstDayOfNextMonth),
      );
    }
  }
  ...
  private updateTimelineHeaders(startDate?: Date, endDate?: Date): void {
    ...
    this.headers = dates.map((date: Date) => ({
      ...
      dayOfWeek: this.capitalizeFirstLetter(
        this.dateUtils.formatDate(date, 'EEEE'),
      ),
    }));
  }
  ...
  private updateButtonsPosition(rect: Rect): void {
    ...
    this.updateButtonPosition(
      this.nextMonthButtonPosition,
      rect.x + rect.width - UNIT,
      rect.y,
    );
  }

  private updateButtonPosition(
    button: ElementPosition,
    x: number,
    y: number,
  ): void {
