...
export class TableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  sort: BehaviorSubject<MatSortable | null> =
    new BehaviorSubject<MatSortable | null>(null);

  private readonly _data = new BehaviorSubject<
    MatTableDataSource<TasksDataSource> | undefined
  >(undefined);
  ...
  constructor(
    ...
    private readonly tasksSorterService: TasksSorterService,
    ...
    return tasks.map((task: Task) => ({
      ...
      startDate:
        task instanceof StartedTask
          ...
          : '-',
      endDate:
        task instanceof EndedTask ? this.formatDate(task.getEndDate()) : '-',
    }));
  }
  ...
  private updateTableDataSource(
    ...
    const array: TasksDataSource[] = tasks
      ? tasks
      : this._data.getValue()?.data || [];

    const filteredArray: TasksDataSource[] = array.filter((item) =>
      this.taskToString(item).includes(search.toLocaleLowerCase()),
    );

    const filteredAndSortedArray: TasksDataSource[] =
      this.tasksSorterService.sort(filteredArray, {
